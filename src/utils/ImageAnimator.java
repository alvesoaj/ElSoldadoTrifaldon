...
public class ImageAnimator {
	...
	public ImageAnimator(ArrayList<BufferedImage> il, int ap, double d, boolean isr) {
		imageList = il;
		periodoAnim = ap;
		duracaoSeq = d;
		estaRepetindo = isr;
	...
	public void updateTick() /*
							 * We assume that this method is called every
							 * animPeriod ms
							 */{
		if (controladorAnim) {
			// update total animation time, modulo the animation sequence
			// duration
			tempTotAnim = (tempTotAnim + periodoAnim)
					% (long) (1000 * duracaoSeq);

			// calculate current displayable image position
			posImg = (int) (tempTotAnim / periodoExibicao) % numImagens; // in
																			// range
																			// 0
																			// to
																			// num-1

			if ((posImg == numImagens - 1) && (estaRepetindo)) { // at end of
																	// sequence
				tempTotAnim = 0L;
			}
		}
	...
	public BufferedImage getCurrentImage() {
		if (numImagens != 0) {
			return (BufferedImage) imageList.get(posImg);
		} else {
			return null;
		}
	...
	public void stop() /*
						 * updateTick() calls will no longer update the total
						 * animation time or imPosition.
						 */{
	...
	public boolean atSequenceEnd() // are we at the last image and not cycling
									// through them?
	{
	...
	public void restartAt(int imPosn) /*
									 * Start showing the images again, starting
									 * with image number imPosn. This requires a
									 * resetting of the animation time as well.
									 */{
		if (numImagens != 0) {
